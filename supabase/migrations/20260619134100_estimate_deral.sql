...
-- 10 · run_estimate_with_enrichment v2: preço-base e comparáveis ancorados no índice
-- regional DERAL/SEAB-PR (por município + categoria derivada do uso MapBiomas), e a
-- camada "comp" passa a reportar a referência DERAL real.
...
begin
  ...
  if v_use_input then
    for r in select value as item from jsonb_array_elements(p_enrichment) loop
      ...
      -- 'comp' é persistido depois com o valor real do DERAL
  ...
  -- categoria DERAL a partir do uso (MapBiomas): lavoura=A, pastagem=B, campo/floresta=C
  ...
  -- preço-base ancorado no índice regional DERAL do município
  ...
  -- comparáveis: municípios reais da mesma região DERAL (valores ancorados)
  ...
  -- fallback sintético se não houve comparáveis DERAL
  ...
  update public.appraisal_estimates
     set comparables_used = (select count(*) from public.comparables where estimate_id = v_est)
   where id = v_est;

  -- snapshot real de comparáveis (DERAL)
  insert into public.data_snapshots (request_id, source_key, source_label, payload)
  ...
  values (p_request_id, v_uid, v_req.status, 'ESTIMATE_DELIVERED', 'run_estimate',
          jsonb_build_object('price_per_ha_avg', v_ppha_avg, 'combined_factor', round(v_combined, 4),
                             'deral', v_comp_real, 'categoria', v_cat));
