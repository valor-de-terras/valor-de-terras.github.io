-- Frente D: motor nível-laudo. Adiciona à estimativa o tratamento estatístico NBR:
-- saneamento por Chauvenet, IC 80% (t-Student) -> Grau de Precisão, e campo de arbítrio.
-- O valor continua GATED (Frente A): IC e arbítrio ficam só no registro persistido e no
-- laudo; a resposta expõe apenas selos não-monetários (grau de fundamentação e precisão).

-- t de Student (bicaudal, alpha=0,20 -> IC 80%) por graus de liberdade.
create or replace function public.t_80(df int)
returns numeric language sql immutable as $$
  select case
    when df <= 0 then 1.282
    when df = 1 then 3.078 when df = 2 then 1.886 when df = 3 then 1.638
    when df = 4 then 1.533 when df = 5 then 1.476 when df = 6 then 1.440
    when df = 7 then 1.415 when df = 8 then 1.397 when df = 9 then 1.383
    when df = 10 then 1.372 when df <= 15 then 1.341 when df <= 20 then 1.325
    when df <= 30 then 1.310 else 1.282 end;
$$;

-- Z crítico de Chauvenet por tamanho da amostra (tabela padrão).
create or replace function public.chauvenet_z(n int)
returns numeric language sql immutable as $$
  select case
    when n <= 2 then 1.15 when n = 3 then 1.38 when n = 4 then 1.54
    when n = 5 then 1.65 when n = 6 then 1.73 when n = 7 then 1.80
    when n = 8 then 1.86 when n = 9 then 1.92 when n = 10 then 1.96
    when n <= 15 then 2.13 when n <= 20 then 2.24 when n <= 25 then 2.33
    else 2.57 end;
$$;

-- Colunas do tratamento estatístico (nullable; preenchidas pela v3).
alter table public.appraisal_estimates
  add column if not exists grau_precisao   text,
  add column if not exists cv              numeric,
  add column if not exists ic80_low        numeric,
  add column if not exists ic80_high       numeric,
  add column if not exists arbitrio_low    numeric,
  add column if not exists arbitrio_high   numeric,
  add column if not exists n_sanitized     integer,
  add column if not exists n_outliers      integer;

-- v3 do motor: mesma base DERAL/homogeneização, mais o tratamento estatístico NBR.
...
begin
  ...
  -- estimativa provisória (base × fatores) — refinada pela estatística após os comparáveis
  ...
  insert into public.appraisal_estimates (
    ...
    round(v_ppha_max * v_prop.area_ha, 2), 'i', 0, v_factors
  ...
  if v_n >= 3 then
    ...
    -- saneamento por Chauvenet (remove outliers |z| > z_crítico)
    ...
    -- Grau de Precisão (amplitude relativa do IC 80%)
    v_prec := case
  ...
  update public.appraisal_estimates set
    ...
    grade = 'i', grau_precisao = v_prec, cv = v_cv,
  ...
  -- Resposta: só selos NÃO monetários (grade/precisão/cv). IC e arbítrio ficam no laudo.
